    ...
    <div className="grid grid-cols-2 gap-6 items-start">
      ...
      <div className="space-y-4 min-w-0 overflow-hidden">
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-4 flex flex-wrap items-center gap-3">
          ...
          <Button onClick={() => handleSave("sent")} loading={saving}>
            Save & Mark as sent
          </Button>
          <Button onClick={handleDownloadPDF} loading={downloadingPdf} variant="secondary">
          ...
          {saveMsg && (
            <span className={`text-sm ${saveMsg.includes("fail") || saveMsg.includes("error") ? "text-red-500" : "text-emerald-600"}`}>
              {saveMsg}
            </span>
          )}
        </div>
