    ...
    <div>
      ...
      <div className="space-y-2">
        {items.map((item) => (
          <div
            key={item.id}
            className="grid grid-cols-1 sm:grid-cols-[1fr_80px_110px_100px_36px] gap-2 items-center"
          >
            <Input
      ...
      <button
        type="button"
        onClick={add}
        className="mt-3 flex items-center gap-1.5 text-xs font-medium text-[#163258] hover:text-[#0f2240] transition-colors"
      >
        <svg className="w-4 h-4" fill="none" stroke="currentColor" strokeWidth={2} viewBox="0 0 24 24">
